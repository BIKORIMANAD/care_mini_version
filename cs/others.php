		...
		<table border=0 style='width:100%'>
			<tr>
				...
				<div id="filter_check" style='max-height: 400px; overflow: auto; padding-top:0px;'>
					...
					<?php
						$sql = "SELECT `sy_product`.`ProductName`,`sy_tarif`.*, in_name.InsuranceName FROM `in_name`,`sy_product`,`sy_tarif` WHERE `sy_tarif`.`InsuranceNameID` = `in_name`.`InsuranceNameID` && `sy_tarif`.`ProductID` = `sy_product`.`ProductID` && `sy_tarif`.`Date` = '{$_GET['date']}' ORDER BY `sy_product`.`ProductName` ASC";
						$query = mysql_query($sql);
					?>
					<table style="background: none; margin-left: auto; margin-right: auto; padding: 5px;text-align: center;" class='list' id=tbl_ref>
						<tr id='th'>
							...
							<th style='width:80px;'>Insurance</th>
						...
						?>
									<tr>
										...
										<td style='width:80px;'><?= $row['InsuranceName'] ?></td>
										<td style='width:80px;'> <?= $row['Amount'] ?></td>
										<td><a href='./others.php?date=<?= @$_GET['date'] ?>&edit=<?= $row['TarifID'] ?>' style='color:blue;'>Edit</a></td>
									</tr>
						<?php			
									$row_++;
								}
