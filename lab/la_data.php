...
$diag = returnAllData("SELECT la_exam.ExamName, la_exam.ExamID FROM la_exam ORDER BY ExamID ASC",$con);
$printed = ""; $i=0;
//var_dump($diag);
if($diag){
	...
	foreach($diag as $d){
		echo "<label><input type=checkbox class=all_data name='{$d['ExamID']}' value='{$d['ExamName']}'>".$d['ExamName']."</label><br />";
	}
}
?>