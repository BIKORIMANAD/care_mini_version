	...
	if($diagnostic){
		...
		for($i=0; $i<count($diagnostic); $i++){
			...
			echo "<tr onclick='$(\".medicine_field\").html(\"{$diagnostic[$i]['MedecineName']}\");$(\"#medicine\").val(\"{$diagnostic[$i]['MedecineNameID']}\");' style='cursor:pointer;' ".($ommit?"title='{$diagnostic[$i]['MedecineName']}'":"").">
				...
				<td align=right>".($diff = $diagnostic[$i]['Quantity'] - $diagnostic[$i]['StockLevel'])."</td>
				<td align=right>".($unit_price = returnSingleField($sql = "SELECT md_price.BuyingPrice FROM md_price WHERE MedecineNameID ='{$diagnostic[$i]['MedecineNameID']}' && Date <= '".(date("Y-m-d",$diagnostic[$i]['Date']))."' ORDER BY Date DESC LIMIT 0, 1","BuyingPrice",true,$con))."</td>
				<td align=right>".(($diff) * $unit_price)."</td>
