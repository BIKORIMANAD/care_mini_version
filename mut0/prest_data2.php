...
if($patients){
	...
	"OTHERS","COST/COUT 100%", "DRUGS/MEDECINES","COST/COUT 100%","TOTAL AMOUNT/MONTANT TOTAL 100% ","CO-PAYMENT /TICKECT MODERATEUR 200 Rwf / 10%","TOTAL AMOUNT TO BE PAID/NET A PAYER");
	...
	<table class=list id=vsbl border="1" style='width:100%; font-size:30px;'>
		...
		for($i=0;$i<count($patients);$i++){
			...
			$kk_ .=  "<tr onclick='$(\".styling\").html(\"<style>#id{$i}{background-color:#e5e5e3;}</style>\");' id='id{$i}'>
				...
				<td>{$ac}</td>
				<td>{$hospit}</td>
				<td>".($zones[(preg_match("#^24030#",$r['InsuranceCardID']))?0:((preg_match("#^240#",$r['InsuranceCardID']))?1:2)])."</td>
			...
			//add the Zone
			$activeSheet->setCellValue(($first_column++).$row, $zones[(preg_match("#^24090[2,3]{1}0#",$r['InsuranceCardID']))?0:((preg_match("#^240#",$r['InsuranceCardID']))?1:2)]);
			//add the Indigent
			...
			$numbers = array($consul_found_count,$lab_rows,$mdo_rows, $hospo_rows,$actso_rows);
