	...
	<table class=list id=vsbl border="1" style='width:100%; font-size:30px;'>
		...
		for($i=0;$i<$last;$i++){
			...
			if(preg_match("#/#",$r['InsuranceCardID'])){
				if($ho_patient){
					(preg_match("#^24030#",$r['InsuranceCardID']))?($iz++):((preg_match("#^240#",$r['InsuranceCardID']))?($ihz++):($ihd++));
				} else{
					(preg_match("#^24030#",$r['InsuranceCardID']))?($oz++):((preg_match("#^240#",$r['InsuranceCardID']))?($ohz++):($ohd++));
				}
			} else{
				if($ho_patient){
					(preg_match("#^24030#",$r['InsuranceCardID']))?($iz++):((preg_match("#^240#",$r['InsuranceCardID']))?($ihz++):($ihd++));
