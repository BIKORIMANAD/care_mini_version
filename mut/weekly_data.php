...
if($patients){
	...
	<table class=list id=vsbl border="1" style='width:100%; font-size:30px;'>
		...
		for($i=0;$i<$last;$i++){
			...
			if($indigent == 0)
				...
				if($ho_patient){
					...
					if(in_array($r['DistrictID'], $zone_districts)){
						$ihz++;
					}
					else{
						$ihd++;
			...
			if($printed != $printed_day || ($i + 1) == $last){
				$total_all = $cons_cost + $med_cost + $labo_cost + $hospitalization_cost + $act_cost + $consumable_cost;
				...
				$total_consumable_cost += $consumable_cost;
				$consumable_cost = 0;
				$act_cost = 0;
		...
		$report_data[] = $total_consumable_cost;
