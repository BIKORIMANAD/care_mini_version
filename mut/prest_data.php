...
if($patients){
	...
	$fheader = array("ID","DATE","AGE","PATIENT'S NAMES","AFFILIATION NUMBER","PATIENT CATEGORY","NEW CASE(NC)",
	"FOLLOW UP(AC)","HOSPITALIZATION","Z,HZ,HD","INDIGENT(YES/NO)","PRISONER(YES/NO)",
	"TRANSFER(YES/NO)","CONSULTATION","COST/COUT 100%","LABORATORY TEST","COST/COUT 100%","MEDICAL IMAGING + COST / COÛT 100%",
	"HOSPITALIZATION","COST/COUT 100%", "PROCEDURES AND MATERIALS", "COST/COUT 100%","AMBULANCE + COST / COÛT 100%", "TRANSFER /TRANSFERT + COST / COÛT 100%",
	"OTHERS CONSUMABLES","COST/COUT 100%", "DRUGS/MEDECINES","COST/COUT 100%","TOTAL AMOUNT/MONTANT TOTAL 100% ","CO-PAYMENT /TICKECT MODERATEUR 200 Rwf / 10%","TOTAL AMOUNT TO BE PAID/NET A PAYER");
	...
	<table class=list id=vsbl border="1" style='width:100%; font-size:30px;'>
		...
		<th>AGE</th><th>PATIENT'S NAMES</th><th>AFFILIATION NUMBER</th><th>PATIENT CATEGORY</th><th>NEW CASE(NC)</th><th>FOLLOW UP(AC)</th><th>HOSPITALIZATION</th><th>Z,HZ,HD</th>
		<th>INDIGENT(YES/NO)</th><th>PRISONER(YES/NO)</th><th>TRANSFER(YES/NO)</th><th>CONSULTATION</th><th>COST/COUT 100%</th><th>LABORATORY TEST</th><th>COST/COUT 100%</th><th>MEDICAL IMAGING + COST / COÛT 100%</th>
		<th>HOSPITALIZATION</th>
		<th>COST/COUT 100%</th>
		<th>PROCEDURES AND MATERIALS</th>
		<th>COST/COUT 100%</th>
		<th>AMBULANCE + COST / COÛT 100%</th>
		<th>TRANSFER /TRANSFERT + COST / COÛT 100%</th>
		<th>OTHERS CONSUMABLES</th>
		<th>COST/COUT 100%</th>
		<th>DRUGS/MEDECINES</th>
		<th>COST/COUT 100%</th>
		<th>TOTAL AMOUNT/MONTANT TOTAL 100% </th>
		<th>CO-PAYMENT /TICKECT MODERATEUR 200 Rwf / 10%</th>
		<th>TOTAL AMOUNT TO BE PAID/NET A PAYER</th></tr>
		...
		for($i=0;$i<count($patients);$i++){
			...
			$hospit = count($ho) >0 ?"Yes":"No";
			
			$nc = $conslt[0]['Amount']==0?"No":"Yes";
			$ac = $conslt[0]['Amount']==0?"Yes":"No";
			$row_count++;
			$zones = array("Z","HZ","HD");
			$kk_ .=  "<tr onclick='$(\".styling\").html(\"<style>#id{$i}{background-color:#e5e5e3;}</style>\");' id='id{$i}'>
				...
				<td>{$ac}</td>
				<td>{$hospit}</td>
				<td>".($zones[(in_array($r['CellID'], $zone_cells))?0:((in_array($r['DistrictID'], $zone_districts))?1:2)])."</td>
			...
			$activeSheet->setCellValue(($first_column++).$row, $patients[$i]["DateofBirth"]);
			...
			//add the IN
			$activeSheet->setCellValue(($first_column++).$row, $hospit);
			//add the Zone
			$activeSheet->setCellValue(($first_column++).$row, ($zones[(in_array($r['CellID'], $zone_cells))?0:((in_array($r['DistrictID'], $zone_districts))?1:2)]));
			//add the Indigent
			...
			//$ddd[] = $patients[$i]['FamilyCode'];
				...
				if($conslt){
					foreach($conslt as $c){
						$consul_found_count++;
						$consltN = $c['Amount'] ==0 ?"No Consultation":$c['ConsultationCategoryName'];
						$consul_found[] = array($consltN,round($c['Amount'],0));
						$cons += $c['Amount'];
						...
						if($medecines){
							foreach($medecines as $l){
								$mdo_rows++;
								$mdo[] = "<td>{$l['MedecineName']}: {$l['Quantity']}</td><td>".($l['Amount'] * $l['Quantity'])."</td>";
								//$mdo_excel[] = array("{$l['MedecineName']}", "{$l['Quantity']}", round($l['Amount'],1), ($l['Amount'] * $l['Quantity']));
								$mdo_content[] = array($l['MedecineName'].":".$l['Quantity'], round(($l['Amount'] * $l['Quantity']),1));
				...
				if(@$consul_found[0]){
					$kk_ .= "<td class='number_right edit{$row_count}1'>{$consul_found[0][0]}</td>";
					$kk_ .= "<td class='number_right edit{$row_count}1'>{$consul_found[0][1]}</td>";
					//$ddd[] = $consul_found[0];
					//add the consultation price on the file
					$activeSheet->setCellValue(($first_column++).$row, $consul_found[0][0]);
					$activeSheet->setCellValue(($first_column++).$row, $consul_found[0][1]);
				...
				if($ho){
					foreach($ho as $c){
						$hospo_rows++;
						$hospo[] = "<td>{$c['Days']} days </td><td class=number_right>".round(($c['Amount'] * $c['Days']),0)."</td>";
				...
				if(@$hospo[0]){
					...
					//add the medical imaging price on the file
					$activeSheet->setCellValue(($first_column++).$row, $hospo_content[0][1]);
				...
				if($consum){
					foreach($consum as $c){
						$actso_rows++;
						$actso[] = "<td>{$c['MedecineName']}:{$c['Quantity']}</td><td class=number_right>".round(($c['Amount'] * $c['Quantity']),1)."</td>";
						$actso_content[] = array($c['MedecineName'].":".$c['Quantity'], round(($c['Amount'] * $c['Quantity']),1));
				...
				//$dd[] = "";
				//add the medical imaging price on the file
				$activeSheet->setCellValue(($first_column++).$row, "0");
			...
			$numbers = array($consul_found_count,$lab_rows,$mdo_rows, $hospo_rows,$actso_rows,$consumo_rows);
			...
			for($n=1;$n<max_value($numbers);$n++){
				...
				if(@$consul_found[$n]){
					$kk_ .= "<td>{$consul_found[$n][0]}</td>";
					$kk_ .= "<td>{$consul_found[$n][1]}</td>";
					//$ddd[] = $consul_found[$n];
					//add the medical imaging price on the file
					$activeSheet->setCellValue(($first_column++).$row, $consul_found[$n][0]);
					$activeSheet->setCellValue(($first_column++).$row, $consul_found[$n][1]);
				...
				if(@$hospo[$n]){
					...
					//add the medical imaging price on the file
					$activeSheet->setCellValue(($first_column++).$row, $hospo_content[$n][0][0]);
					$activeSheet->setCellValue(($first_column++).$row, $hospo_content[$n][0][1]);
					//add the medical imaging price on the file
		...
		$activeSheet->getStyle("A5:AE".(--$row))->applyFromArray($styleArray);
