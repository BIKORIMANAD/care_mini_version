...
if(@$_POST['print_bill']){
	...
	if(@$_POST['principal_existbefore']){
		...
		if(!@$_POST['principaldiagnostic']){
			...
			if(!$diag_id){
				saveAndReturnID("INSERT INTO co_diagnostic SET DiagnosticName='".PDB($_POST['principaldiagnostic'],true,$con)."', DiagnosticCategoryID=10, PECIME=0, Reported=0, SISCode='', Code='', DiagnosticCode='', English=''",$con);
			}
	...
	foreach($diag_data as $exam){
		...
		foreach($exam as $key=>$value){
			...
			if(!$d_id = returnSingleField("SELECT DiagnosticID from co_diagnostic WHERE DiagnosticName='".PDB($value,true,$con)."' && DiagnosticCode !=''",$field="DiagnosticID",$data=true, $con)){
				// $break = true;
				$d_id = saveAndReturnID("INSERT INTO co_diagnostic SET DiagnosticName='".PDB($value,true,$con)."', DiagnosticCategoryID=10, PECIME=0, Reported=0, SISCode='', Code='', DiagnosticCode='', English=''",$con);
				// break;
			}
