...
<script>
	$("#medecinename<?= $_GET['number'] ?>").keyup(function(e){
		...
		var ichtyoleePattern = /ichtyolee/;
		var pommadePattern = /pommade/;
		var CAFPattern = /CAF/;
		if(pattern.test($("#medecinename<?= $_GET['number'] ?>").val() )){
			$("#medecinequantity<?= $_GET['number'] ?>").val("1");
		} else if(siropPattern.test($("#medecinename<?= $_GET['number'] ?>").val() )){
			$("#medecinequantity<?= $_GET['number'] ?>").val("1");
		} else if(ichtyoleePattern.test($("#medecinename<?= $_GET['number'] ?>").val() )){
			$("#medecinequantity<?= $_GET['number'] ?>").val("0.25");
		} else if(pommadePattern.test($("#medecinename<?= $_GET['number'] ?>").val() )){
			$("#medecinequantity<?= $_GET['number'] ?>").val("1");
		} else if(CAFPattern.test($("#medecinename<?= $_GET['number'] ?>").val() )){
			$("#medecinequantity<?= $_GET['number'] ?>").val("1");
		} else{
	...
	$("#medecinequantity<?= $_GET['number'] ?>").keyup(function(){
		var qty = $(this).val();
		// Check if the Medicine is one of the predefined Ones
		var pattern = /^coartem/;
		var siropPattern = /sirop/;
		var ichtyoleePattern = /ichtyolee/;
		var pommadePattern = /pommade/;
		...
		if(qty > 2){
			...
			if(pattern.test($("#medecinename<?= $_GET['number'] ?>").val() )){
				// $("#medecinequantity<?= $_GET['number'] ?>").val("1");
				mdName = $("#medecinename<?= $_GET['number'] ?>").val();
			} else if(siropPattern.test($("#medecinename<?= $_GET['number'] ?>").val() )){
				mdName = $("#medecinename<?= $_GET['number'] ?>").val();
			} else if(ichtyoleePattern.test($("#medecinename<?= $_GET['number'] ?>").val() )){
				mdName = $("#medecinename<?= $_GET['number'] ?>").val();
			...
			if(mdName){
				alert("It is abnormal to have " + qty + "\nof " + mdName + "\nPlease Verify the provided data.\n")
				if(ichtyoleePattern.test($("#medecinename<?= $_GET['number'] ?>").val() )){
					$("#medecinequantity<?= $_GET['number'] ?>").val("0.25");
				} else {
					$("#medecinequantity<?= $_GET['number'] ?>").val("1");
				}
