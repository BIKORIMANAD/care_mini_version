...
//var_dump($_GET);
if(strlen($_GET['key'])){
	$_GET['key'] = PDB($_GET['key'], true,$con);
	...
	<table class=list id=vsbl border="1" style='width:100%;'>
		<tr><th colspan=2>&nbsp;</th><th>#</th><th>Date</th><th>Service</th><th>TM</th><th>Cat.</th><th>Name</th><th>Beneficiary ID</th><th>Age</th><th>Sex</th><th>Family</th><th>Householder ID</th><th>Weight</th><th>Temp.</th><th>District</th><th>Sector</th><th>Cell</th><th>Village</th></tr>
		<?php
		$service = null;
		for($i=0;$i<count($records);$i++){
			...
			echo "<tr id='id{$i}'>
					...
					<td class='pa_{$i}10' onclick='if(true){ return false;} if($(\"#edit_mode\").val() == 0){ edit_function(\"pa_{$i}9\",\"{$records[$i]['TicketPaid']}\",\"mu_tm\",\"{$records[$i]['TicketID']}\",\"TicketID\",\"TicketPaid\",\"save_rqst/save_pa_info.php\");}'>{$records[$i]["FamilyCategory"]}</td>
					...
					<td class='pa_{$i}11' onclick='if($(\"#edit_mode\").val() == 0){ edit_function(\"pa_{$i}11\",\"{$records[$i]['NationalID']}\",\"pa_records\",\"{$records[$i]['PatientRecordID']}\",\"PatientRecordID\",\"NationalID\",\"save_rqst/save_pa_info.php\");}'>{$records[$i]["NationalID"]}</td>
