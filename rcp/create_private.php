...
function OTHER(){
	$(".tm_view").html("<label style='display:none;'><input type=radio id='paid' name='tm' value='0' checked /> Paid <?= $tm_to_pay ?> RWF</label><input type='hidden' value='OK' placeholder='Receipt Number' id=receipt_number name=receipt_number class=txtfield1 style='width:70px;' />");
	$(".fcategory").html("");
	$(".affection_location").html("");
	$(".affection_locationdata").html("");
	$(".h_label").html("House Manager <label class=as_link onclick='$(\"#father\").val($(\"#name\").val())'>Self</label> <label class=error-text>*</label>");
