...
if($_GET['format'] == "excel" && @$_SESSION['report']){
	...
	//echo $file_name;
		...
		//echo count($data[0]);
		$last_column = count($data[0]) == 19?"S":(count($data[0]) == 18?"R":(count($data[0]) == 16?"P":(count($data[0]) == 21?"U":"A")));
		$first_column = "A";
		...
		// SpanCells($activeSheet,"A".($row + 3).":B".($row + 3),$align='left');
		$activeSheet->setCellValue("A".($row + 3), "PREPARED BY: ");
		$activeSheet->setCellValue("A".($row + 6), "");
		$activeSheet->setCellValue("A".($row + 8), "Receptionist.");

		$activeSheet->setCellValue("E".($row + 3), "VERIFIED BY: "."");
		$activeSheet->setCellValue("E".($row + 6), "");
		$activeSheet->setCellValue("E".($row + 8), " ACCOUNTANT OF Health Center. "."");

		$activeSheet->setCellValue("J".($row + 3), "APPROVED BY: "."");
		$activeSheet->setCellValue("J".($row + 6), "");
		$activeSheet->setCellValue("J".($row + 8), "HEAD OF Health Center. "."");
		...
		$objWriter->save("./facture/{$_GET['in']}_".$file_name);
...
<html>
	...
	<body>
<h2>Available report please select one</h2>
				...
				if(count($Dir->TabFiles) >0){
					...
					foreach ($Dir->TabFiles as $f) {
						...
						//if(preg_match("/^".preg_replace("/ /","_",trim($db->select1cell("school_report_db`.`tbl_users","Name",array("ID"=>$_SESSION['u_id']),true)))."/",$f['filename']))
							$out .= "<tr><td>[".$f['extension']."]</td><td>".strtoupper(explode("_",$f['filename'])[0])."</td><td><a style='color:blue; text-decoration:none;' target='_blank' href='./".$f['dirname']."/".$f['filename']."'>Created on ".date("Y-m-d H:i:s",$f['datechange'])."</a></td><td>".round($f['size']*(1/1024),1)." KB</td><!--<td align=right>".date('Y-m-d h:i:s',$f['datemodify'])."</td>--></tr>";
