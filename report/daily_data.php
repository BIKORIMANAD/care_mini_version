...
if($patients || !$patients){
	...
	//echo $sql;
	$amount_consultation = returnSingleField($sql="SELECT SUM(`Amount`) as ConsulatationPrice FROM co_price, co_records, pa_records, in_category WHERE pa_records.PatientRecordID = co_records.PatientRecordID && co_records.ConsultationPriceID = co_price.ConsultationPriceID && co_price.InsuranceCategoryID = in_category.InsuranceCategoryID && in_category.InsuranceCode = 'D' && pa_records.DateIn='{$date}'","ConsulatationPrice",true,$con);//$private_records_amount['ConsulatationPrice'];
	//echo $sql;
	$amount_labo = returnSingleField($sql="SELECT SUM(`Amount`) as LaboPrice FROM la_price, la_records, co_records, pa_records, in_category WHERE pa_records.PatientRecordID = co_records.PatientRecordID && co_records.ConsultationRecordID = la_records.ConsultationRecordID && la_records.ExamPriceID = la_price.ExamPriceID && la_price.InsuranceTypeID = in_category.InsuranceCategoryID && in_category.InsuranceCode = 'D' && pa_records.DateIn='{$date}'","LaboPrice",true,$con);//$private_records_amount['ConsulatationPrice'];
	...
	$cn_records = formatResultSet($rslt=returnResultSet($sql="SELECT md_price.Amount, cn_records.Quantity FROM md_price, cn_records, pa_records, in_name WHERE pa_records.PatientRecordID = cn_records.PatientRecordID && cn_records.MedecinePriceID = md_price.MedecinePriceID && pa_records.InsuranceNameID = in_name.InsuranceNameID && in_name.InsuranceName = 'Private' && pa_records.DateIn='{$date}'",$con),true,$con);//$private_records_amount['ConsulatationPrice'];
	//var_dump($md_records);
	//loop all medicines found
	...
	<table border=0 class=no-out-border style='margin:2px;'>
		...
		<tr><td>PH + Facture</td>
			...
			<td id=number> <?= $cbhi_patient_ph *= 40; ?></td>
			<?php
