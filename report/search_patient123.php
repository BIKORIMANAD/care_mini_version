...
if($patients){
	...
	<table class=list id=vsbl border="1" style='width:100%; font-size:30px;'>
		<tr><th>N<sup>o</sup></th><th>Date</th><th>Service</th><th>Cat.</th><th>Name</th><th>ID Number</th><th>Age</th><th>Sex</th><th>House Holder</th><th>ID Number of Household</th><th>Cons Cost</th><th>Lab</th><th>Imaging</th><th>Hosp.</th><th>Proc & Mat.</th><th>Ambul.</th><th>Consum.</th><th>Drugs.</th><th>Total</th><th>Co-payment</th><th>Amount after verif.</th></tr>
		<tr><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th><th>&nbsp;</th><th>100%</th><th>100%</th><th>100%</th><th>100%</th><th>100%</th><th>100%</th><th>100%</th><th>100%</th><th>100%</th><th>200 RWF / 10%</th><th></th></tr>
		<?php
		$t = array();
		...
		for($i=0;$i<count($patients);$i++){
			...
			$housemanager = $r['NationalID'];
			$beneficiary = $r["InsuranceCardID"];

			if ($housemanager == null) {
				$housemanager = $beneficiary;
			}else{
				$housemanager = $r['NationalID'];
				$beneficiary = $r["InsuranceCardID"];
			}
			...
			echo "<tr onclick='$(\".styling\").html(\"<style>#id{$i}{background-color:#e5e5e3;}</style>\");' id='id{$i}'>
				...
				<td>".str_replace("/", "", $beneficiary)."</td>
				<td>".($patients[$i]["DateofBirth"] == "0000-00-00"?"":$patients[$i]["DateofBirth"])."</td>
				<td>{$patients[$i]["Sex"]}</td>
				<td>{$patients[$i]["FamilyCode"]}</td>
				<td>".str_replace("/", "", $housemanager)."</td>";
				$ddd[] = ($i+1);
				$ddd[] = $r["DateIn"];
				$ddd[] = $r["ServiceCode"];
				$ddd[] = $r['FamilyCategory'];
				$ddd[] = $r["Name"];
				$ddd[] = str_replace("/", "", $beneficiary).(is_numeric(str_replace("/", "", $beneficiary))?" ":"");
				$ddd[] = $r["DateofBirth"];
				$ddd[] = $r["Sex"];
				$ddd[] = $r["FamilyCode"];
				$ddd[] = str_replace("/", "", $housemanager)." ";
