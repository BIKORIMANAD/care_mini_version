...
if(strlen($_POST['date'])){
	...
	$patients = formatResultSet($rslt=returnResultSet($sql="SELECT DISTINCT in_name.InsuranceName, pa_records.PatientRecordID, pa_records.FamilyCategory, pa_records.DateIn, pa_records.InsuranceCardID, se_name.ServiceCode, pa_info.*, ad_village.*, ad_cell.*, ad_sector.*, ad_district.* FROM in_name, pa_records, pa_info, se_name, se_records, ad_village, ad_cell, ad_sector, ad_district, sy_users, sy_center WHERE pa_info.PatientID = pa_records.PatientID && pa_records.InsuranceNameID = in_name.InsuranceNameID && pa_info.VillageID=ad_village.ViillageID && ad_village.CellID=ad_cell.CellID && ad_cell.SectorID=ad_sector.SectorID && ad_sector.DistrictID=ad_district.DistrictID && pa_records.ReceptionistID = sy_users.UserID && sy_users.CenterID = sy_center.CenterID && {$sys} && DateIn = '{$_POST['date']}' && se_records.PatientRecordID = pa_records.PatientRecordID && se_records.ServiceNameID = se_name.ServiceNameID ORDER BY pa_records.DateIn ASC, pa_records.PatientRecordID ASC",$con),$multirows=true,$con); // && pa_records.Status != 0 
	//var_dump($patients);
	//echo $sql;
	if($patients){
